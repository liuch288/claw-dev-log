...
|------|---------|------|-----|----------|----------|------|------|------|
| 2026-04-23 | CLQ-118 | factor_view | [CLQ-118: Add main contract display features](https://github.com/liuch288/factor_view/pull/8) | ✅ 可合并 | 0 | 1 | 2 | 主力合约（"主力合约"选项）功能实现完整：GuidePage 新增"主力合约"选项，API 支持 mode=by-product，App.vue 透传 instrument 显示；start.sh 修复 conda run -n quantdev；警告：catalog.py 注释块重复（两个 "# Directory traversal helpers"）；轻微：dominantMap 命名可考虑更明确（如 dominantContractsMap）；GuidePage.vue selectInstrument 参数 inst 可能为 null（line 245）未处理 |
