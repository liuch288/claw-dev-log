...
|------|---------|------|-----|----------|----------|------|------|------|
| 2026-04-25 | CLQ-126 | contextum | [CLQ-126: Add password protection to dashboard](https://github.com/liuch288/contextum/pull/11) | ✅ 可合并 | 0 | 1 | 1 | Dashboard 新增密码保护：login_required 装饰器、登录/登出路由、30天 session；默认密码 admin123；警告：硬编码默认密码 admin123 风险高，建议首次启动强制修改或禁止默认密码；建议：FLASK_SECRET_KEY 默认 urandom(24).hex() 每次重启不同，session 无法跨重启保持，建议从环境变量读取或固定值 |
