...
|------|---------|------|-----|----------|----------|------|------|------|
| 2026-04-17 | CLQ-77 | lrbt | [CLQ-77: Add basic DMU implementations](https://github.com/liuch288/lrbt/pull/2) | ❌ 需修复 | 2 | 2 | 1 | PR描述与实际代码严重不符：描述称添加SimpleTrendDMU/MeanRevertDMU/BreakoutDMU，实际代码为AtrDMU/KlinePatternDMU；setup.py版本0.1→0.0.1降级 | | ⚠️ 建议修改 | 0 | 2 | 2 | 多日计算核心逻辑完整，测试覆盖充分（314行）；主要问题：_generate_date_range 包含所有日历日（含周末/假日），非交易日会触发不必要的失败；cli.py 参数验证与 core.py 重复，可提取公共方法；fail_fast + 日终重置逻辑正确 |
